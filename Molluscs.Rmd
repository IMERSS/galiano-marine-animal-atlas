...
library(tidyverse)
molluscs <- read.csv("tabular_data/molluscs_records_gridded.csv")
mollusc.summary <- read.csv("tabular_data/molluscs_summary.csv")
...
With about 49,000 extant species, Mollusca are the second largest phylum after Arthropoda. The phylum comprises seven extant classes, five of which are locally represented around Galiano Island, including bivalves, cephalopods, gastropods, chitons, and tusk shells. Over 780 marine mollusc species have been identified in British Columbia. Of these, `r nrow(mollusc.summary)` species have been reported for Galiano Island over the last century and a half, through the combined efforts of more than `r length(unique(molluscs$recordedBy))` individuals.
...
animal.grid.map <- mx_diversity_map("molluscs")
animal.grid.map
```
...
Please help contribute to the growing record of the island mollusc diversity by submitting your observations to the [Biodiversity Galiano iNaturalist project.](http://inaturalist.org/projects/biodiversity-galiano-island) Here, you can browse photos of mollusc species commonly documented around Galiano Island, as well as recent observations, most favourited observations, and top observers, based on [iNaturalist](http://inaturalist.org/) data. 
