# Map reporting status of Galiano Island's cnidarian species
...
cnidarians <- read.csv("tabular_data/Galiano_marine_animal_records_consolidated_2023-04-22.csv")

summary <- read.csv("tabular_data/cnidarian_summary.csv")
...
confirmed.taxa.records <- cnidarians %>% filter(str_detect(scientificName, confirmed.taxa))
confirmed.taxa.records$status <- 'confirmed'

new.taxa.records <- cnidarians %>% filter(str_detect(scientificName, new.taxa))
new.taxa.records$status <- 'new'

reported.taxa.records <- cnidarians %>% filter(str_detect(scientificName, reported.taxa))
...
# Read gridded marine cnidarians dataset

cnidarians.confirmed.gridded <- read.csv("tabular_data/cnidarians_confirmed_records_gridded.csv")
cnidarians.new.gridded <- read.csv("tabular_data/cnidarians_new_records_gridded.csv")
cnidarians.reported.gridded <- read.csv("tabular_data/cnidarians_reported_records_gridded.csv")

# Summarize fish by grid cell and export to JSON file for viz

gridded.cnidarians.confirmed <- cnidarians.confirmed.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

gridded.cnidarians.new <- cnidarians.new.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

gridded.cnidarians.reported <- cnidarians.reported.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

write(jsonlite::toJSON(gridded.cnidarians.confirmed), "viz_data/fishconfirmedGridCellData.json")
write(jsonlite::toJSON(gridded.cnidarians.new), "viz_data/fishnewGridCellData.json")
write(jsonlite::toJSON(gridded.cnidarians.reported), "viz_data/fishReportedGridCellData.json")

# Load choropleths

gridded.confirmed.records <- mx_read("spatial_data/vectors/cnidarians_confirmed")
gridded.new.records <- mx_read("spatial_data/vectors/cnidarians_new")
gridded.historical.records <- mx_read("spatial_data/vectors/cnidarians_reported")
...
# Strange structure to mirror that in cnidarians
