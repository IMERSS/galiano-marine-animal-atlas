# Map reporting status of Galiano Island's flatworm species
...
platyhelminthes <- read.csv("tabular_data/Galiano_marine_animal_records_consolidated_2023-04-22.csv")

summary <- read.csv("tabular_data/platyhelminthe_summary.csv")
...
confirmed.taxa.records <- platyhelminthes %>% filter(str_detect(scientificName, confirmed.taxa))
confirmed.taxa.records$status <- 'confirmed'

new.taxa.records <- platyhelminthes %>% filter(str_detect(scientificName, new.taxa))
new.taxa.records$status <- 'new'

reported.taxa.records <- platyhelminthes %>% filter(str_detect(scientificName, reported.taxa))
reported.taxa.records$status <- 'historical'

records <- rbind(new.taxa.records,confirmed.taxa.records,reported.taxa.records)

# Summarise platyhelminthe species by reporting status

taxa.status <- records %>% group_by(status) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

# Read gridded marine platyhelminthe dataset

platyhelminthes.confirmed.gridded <- read.csv("tabular_data/platyhelminthes_confirmed_records_gridded.csv")
platyhelminthes.new.gridded <- read.csv("tabular_data/platyhelminthes_new_records_gridded.csv")
platyhelminthes.reported.gridded <- read.csv("tabular_data/platyhelminthes_reported_records_gridded.csv")

# Summarize platyhelminthe by grid cell and export to JSON file for viz

gridded.platyhelminthes.confirmed <- platyhelminthes.confirmed.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

gridded.platyhelminthes.new <- platyhelminthes.new.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

gridded.platyhelminthes.reported <- platyhelminthes.reported.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

write(jsonlite::toJSON(gridded.platyhelminthes.confirmed), "viz_data/platyhelmintheconfirmedGridCellData.json")
write(jsonlite::toJSON(gridded.platyhelminthes.new), "viz_data/platyhelminthenewGridCellData.json")
write(jsonlite::toJSON(gridded.platyhelminthes.reported), "viz_data/platyhelmintheReportedGridCellData.json")

# Load choropleths

gridded.confirmed.records <- mx_read("spatial_data/vectors/platyhelminthes_confirmed")
gridded.new.records <- mx_read("spatial_data/vectors/platyhelminthes_new")
gridded.historical.records <- mx_read("spatial_data/vectors/platyhelminthes_reported")
...
# Strange structure to mirror that in platyhelminthes
...
write(rjson::toJSON(statusData), "viz_data/Status-platyhelminthePlotData.json")
