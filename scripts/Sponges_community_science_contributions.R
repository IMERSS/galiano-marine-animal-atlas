# Map reporting status of Galiano Island's sponge species
...
sponges <- read.csv("tabular_data/Galiano_marine_animal_records_consolidated_2023-04-22.csv")

summary <- read.csv("tabular_data/sponge_summary.csv")
...
confirmed.taxa.records <- sponges %>% filter(str_detect(scientificName, confirmed.taxa))
confirmed.taxa.records$status <- 'confirmed'

new.taxa.records <- sponges %>% filter(str_detect(scientificName, new.taxa))
new.taxa.records$status <- 'new'

reported.taxa.records <- sponges %>% filter(str_detect(scientificName, reported.taxa))
...
# Read gridded marine sponges dataset

sponges.confirmed.gridded <- read.csv("tabular_data/sponges_confirmed_records_gridded.csv")
sponges.new.gridded <- read.csv("tabular_data/sponges_new_records_gridded.csv")
sponges.reported.gridded <- read.csv("tabular_data/sponges_reported_records_gridded.csv")

# Summarize fish by grid cell and export to JSON file for viz

gridded.sponges.confirmed <- sponges.confirmed.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

gridded.sponges.new <- sponges.new.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))
...
gridded.confirmed.records <- mx_read("spatial_data/vectors/sponges_confirmed")
gridded.new.records <- mx_read("spatial_data/vectors/sponges_new")
gridded.historic.records <- mx_read("spatial_data/vectors/sponges_reported")
