# Map reporting status of Galiano Island's marine fish species
...
fishes <- read.csv("tabular_data/Galiano_marine_animal_records_consolidated_2023-04-22.csv")

summary <- read.csv("tabular_data/fishes_summary.csv")
...
confirmed.taxa.records <- fishes %>% filter(str_detect(scientificName, confirmed.taxa))
confirmed.taxa.records$status <- 'confirmed'

new.taxa.records <- fishes %>% filter(str_detect(scientificName, new.taxa))
new.taxa.records$status <- 'new'

reported.taxa.records <- fishes %>% filter(str_detect(scientificName, reported.taxa))
...
# Read gridded marine fishes dataset

fishes.confirmed.gridded <- read.csv("tabular_data/fishes_confirmed_records_gridded.csv")
fishes.new.gridded <- read.csv("tabular_data/fishes_new_records_gridded.csv")
fishes.reported.gridded <- read.csv("tabular_data/fishes_reported_records_gridded.csv")

# Summarize fish by grid cell and export to JSON file for viz

gridded.fishes.confirmed <- fishes.confirmed.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

gridded.fishes.new <- fishes.new.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

gridded.fishes.reported <- fishes.reported.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

write(jsonlite::toJSON(gridded.fishes.confirmed), "viz_data/fishconfirmedGridCellData.json")
write(jsonlite::toJSON(gridded.fishes.new), "viz_data/fishnewGridCellData.json")
write(jsonlite::toJSON(gridded.fishes.reported), "viz_data/fishReportedGridCellData.json")

# Load choropleths

gridded.confirmed.records <- mx_read("spatial_data/vectors/fishes_confirmed")
gridded.new.records <- mx_read("spatial_data/vectors/fishes_new")
gridded.historical.records <- mx_read("spatial_data/vectors/fishes_reported")
...
# Strange structure to mirror that in fishes
