# Map reporting status of Galiano Island's nemertean species
...
nemerteans <- read.csv("tabular_data/Galiano_marine_animal_records_consolidated_2023-04-22.csv")

summary <- read.csv("tabular_data/nemertean_summary.csv")
...
confirmed.taxa.records <- nemerteans %>% filter(str_detect(scientificName, confirmed.taxa))
confirmed.taxa.records$status <- 'confirmed'

new.taxa.records <- nemerteans %>% filter(str_detect(scientificName, new.taxa))
new.taxa.records$status <- 'new'

reported.taxa.records <- nemerteans %>% filter(str_detect(scientificName, reported.taxa))
...
nemerteans.confirmed.gridded <- read.csv("tabular_data/nemerteans_confirmed_records_gridded.csv")
nemerteans.new.gridded <- read.csv("tabular_data/nemerteans_new_records_gridded.csv")
nemerteans.reported.gridded <- read.csv("tabular_data/nemerteans_reported_records_gridded.csv")

# Summarize mollusc by grid cell and export to JSON file for viz

gridded.nemerteans.confirmed <- nemerteans.confirmed.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

gridded.nemerteans.new <- nemerteans.new.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

gridded.nemerteans.reported <- nemerteans.reported.gridded %>% group_by(cell_id) %>% 
  summarize(taxa = paste(sort(unique(scientificName)),collapse=", "))

write(jsonlite::toJSON(gridded.nemerteans.confirmed), "viz_data/molluscconfirmedGridCellData.json")
write(jsonlite::toJSON(gridded.nemerteans.new), "viz_data/molluscnewGridCellData.json")
write(jsonlite::toJSON(gridded.nemerteans.reported), "viz_data/molluscReportedGridCellData.json")

# Load choropleths

gridded.confirmed.records <- mx_read("spatial_data/vectors/nemerteans_confirmed")
gridded.new.records <- mx_read("spatial_data/vectors/nemerteans_new")
gridded.historical.records <- mx_read("spatial_data/vectors/nemerteans_reported")
...
# Strange structure to mirror that in nemerteans
